...
// The generator is release-agnostic: a later release points it at its own
// manifest and artifact path, so the catalogue shape can never fork.
const source = resolve(root, argValue("--input", "content/v6/manifest.authoring.json"));
const output = resolve(root, argValue("--output", "src/generated/release-catalog.ts"));
const generator = argValue("--generator", "scripts/v6/build-client-catalog.mjs");
...
const foodItems = items
  ...
  .sort((left, right) => left.tier_index - right.tier_index);
if (foodItems.length !== 16) throw new Error(`Expected 16 foods, got ${foodItems.length}`);
const foods = foodItems.map((item, index) => {
  if (item.tier_index !== index + 1) throw new Error(`Food ladder has no tier ${index + 1}`);
  if (!(item.stats.heal > 0)) throw new Error(`Food ${item.id} heals nothing`);
  if (index > 0 && item.stats.heal <= foodItems[index - 1].stats.heal) {
    throw new Error(`Food healing is not strictly increasing at tier ${item.tier_index}`);
  }
  const recipe = recipes.find((entry) => entry.out === item.id);
  if (!recipe) throw new Error(`Food ${item.id} has no recipe`);
  if (recipe.skill !== "cooking") throw new Error(`Food ${item.id} is not cooked`);
  return {
    id: item.id,
...
const generated =
  ...
  `export const RELEASE_FOODS = ${JSON.stringify(foods, null, 2)} as const;\n\n` +
  ...
  `export const RELEASE_FOOD_BY_ID = Object.fromEntries(RELEASE_FOODS.map((entry) => [entry.id, entry])) as Record<string, (typeof RELEASE_FOODS)[number]>;\n` +
  `export const BASE_ATTACK_INTERVAL_S = 0.85;\n`;

if (checkOnly) {
  if ((await readFile(output, "utf8").catch(() => "")) !== generated) {
    throw new Error(`Client release catalog drifted; run node ${generator}`);
  }
  console.log(
    `Verified client catalog (${items.length} items, ${recipes.length} recipes, ${armour.length} armour, ${weapons.length} swords, ${potions.length} potions, ${foods.length} foods)`,
  );
} else {
  await writeFile(output, generated);
  console.log(
    `Wrote ${output.slice(root.length + 1)} (${items.length} items, ${recipes.length} recipes, ${armour.length} armour, ${weapons.length} swords, ${potions.length} potions, ${foods.length} foods)`,
  );
}
