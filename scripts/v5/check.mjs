...
check(
  world.spawns.length === v4.runtime.node_spawns.length + v4.runtime.monster_spawns.length,
...
check(
  weapons.every((weapon) => weapon.active),
  "every published sword must be active",
);
check(
  JSON.stringify(weapons.map((weapon) => weapon.id).sort()) ===
    JSON.stringify([...SWORD_IDS].sort()),
  "sword ids drifted from the approved stable id set",
);
check(
  new Set(weapons.map((weapon) => weapon.tier_index)).size === 16,
  "swords are not one per tier",
);
for (const sword of SWORDS) {
  ...
  check(
    ...
    `sword ${sword.id} is named "${item.name}", expected "${sword.name}"`,
  );
  ...
  check(
    item.level_requirement === before.level_requirement,
    `sword ${sword.id} level requirement changed from v4`,
  );
}
...
for (let index = 1; index < ladder.length; index += 1) {
  check(
    ladder[index].stats.attack > ladder[index - 1].stats.attack,
...
check(
  BASE_ATTACK_INTERVAL_S > 0 && BASE_ATTACK_INTERVAL_S <= 2,
  "base attack interval is out of range",
);
...
for (const recipe of swordRecipes) {
  ...
  check(
    ...
    `recipe ${recipe.id} changed; V5 must not rebalance crafts`,
  );
  check(
    recipe.station === "forge" && recipe.skill === "smithing",
    `recipe ${recipe.id} is not a Smithing forge craft`,
  );
  check(recipe.inputs.length > 0, `recipe ${recipe.id} has no ingredients`);
  for (const input of recipe.inputs) {
    check(
      itemIds.has(input.item_id),
      `recipe ${recipe.id} consumes undefined item ${input.item_id}`,
    );
    check(
      obtainable.has(input.item_id),
      `recipe ${recipe.id} consumes unobtainable item ${input.item_id}`,
    );
    check(
      input.qty > 0,
      `recipe ${recipe.id} consumes a non-positive quantity of ${input.item_id}`,
    );
  }
}
...
check(
  stopRules.length === DELETED_ITEMS.length,
  "the deletion ledger does not name all four tester weapons",
);
check(
  stopRules.every((rule) => rule.action === "stop" && !rule.to_id),
...
for (const [name, sql] of Object.entries(migrations)) {
  ...
  check(
    ...
    `${name} migration performs a destructive schema change`,
  );
}
const activate = migrations.activate;
check(
  activate.includes("V5 activation expects v4 to be the active release"),
  "activation does not require v4 active",
);
check(
  activate.includes("game_validate_content_version('v5')"),
  "activation does not validate v5 before cleanup",
);
check(
  activate.indexOf("game_validate_content_version('v5')") <
    activate.indexOf("INSERT INTO public.player_save_backups"),
...
check(
  activate.includes("DELETE FROM public.market_listings WHERE item_id = ANY"),
  "activation does not clear market listings",
);
check(
  activate.includes("DELETE FROM public.market_prices WHERE item_id = ANY"),
  "activation does not clear price history",
);
check(
  activate.includes("still reference a deleted weapon id"),
  "activation does not assert saves are clean",
);
check(activate.includes("does not define"), "activation does not assert saves still deserialize");
check(
  activate.includes("'{weapon}', 'null'::jsonb"),
  "activation does not clear the equipped weapon slot",
);
check(!activate.includes("'{armor}'"), "activation must not touch armour slots");
check(
  activate.includes("v4 must be retained as retired for rollback".replace("v4 must", "v4 must")) ||
    activate.includes("must be retained as retired for rollback"),
  "activation does not retain v4 for rollback",
);
check(
  !/DELETE FROM public\.game_content_\w+\s+WHERE content_version = 'v4'/.test(activate),
...
const releaseItems = JSON.parse(
  catalog.slice(
    catalog.indexOf("RELEASE_ITEMS = ") + 16,
    catalog.indexOf("] as const;\nexport const RELEASE_RECIPES") + 1,
  ),
);
const clientWeapons = releaseItems.filter((item) => item.kind === "weapon");
check(
  clientWeapons.length === 16,
  `client catalog publishes ${clientWeapons.length} swords, expected 16`,
);
for (const sword of SWORDS) {
...
const clientSources = await Promise.all(
  [
    ...
    "tests/fixtures/save-fixtures.ts",
...
check(
  dialog.includes("releaseWeaponTiers()"),
  "the Weaponsmith does not render the generated sword ladder",
);
check(
  dialog.includes("BASE_ATTACK_INTERVAL_S"),
...
check(
  /grid-cols-2 gap-1\.5/.test(dialog),
  "the Weaponsmith ladder is not a responsive two-column grid",
);
check(
  dialog.includes("+2% attack per level through +50, then +0.5% per level"),
  "the upgrade explanation was not corrected",
);
const data = await read("src/game/data.ts");
check(
  data.includes("export const PLUS_STEP = 0.02;") && data.includes("PLUS_STEP_ABOVE_50 = 0.005"),
