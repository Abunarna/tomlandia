CREATE TABLE public.world_boss (
  ...
  CONSTRAINT world_boss_single CHECK (id = 1)
);

GRANT SELECT ON public.world_boss TO authenticated;
GRANT ALL ON public.world_boss TO service_role;

ALTER TABLE public.world_boss ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Anyone signed in can watch the world boss"
  ON public.world_boss FOR SELECT TO authenticated USING (true);

INSERT INTO public.world_boss (id, hp, max_hp) VALUES (1, 45000, 45000);

ALTER TABLE public.world_boss REPLICA IDENTITY FULL;
ALTER PUBLICATION supabase_realtime ADD TABLE public.world_boss;
...
BEGIN
  ...
  -- His position is computed client-side from a shared deterministic path, so
  -- the caller reports it. Sanity-check it against the last position any player
  -- reported: he can only have moved so far in the meantime.
  ...
  IF sqrt(power(_x - _bx, 2) + power(_y - _by, 2)) > 220 THEN
  ...
  -- he always swings back at anyone standing in his reach
...
REVOKE ALL ON FUNCTION public.attack_boss(numeric, numeric, numeric, numeric, boolean) FROM public;
