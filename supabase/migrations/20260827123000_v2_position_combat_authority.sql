-- V2 combat/position authority repair.
-- Generated from the audited production function, with one deliberate change:
-- each swing first commits the bounded client position through track_position.
...
BEGIN
  ...
  -- Accept the same bounded movement contract used by background presence,
  -- then consume that authoritative row for this swing. Position and combat
  -- now share one transaction, so a visually adjacent target cannot be judged
  -- against an older login coordinate.
